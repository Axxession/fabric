...
export default function DesfireStudioPage() {
  ...
  if (auth.isLoading || auth.activeNavigator) {
    return <div className="rounded-structural border border-border bg-content p-6 text-[14px] text-muted-foreground">{t('desfireStudio.loading')}</div>;
  }
    ...
    <section className="grid w-full gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-stretch">
      <div className="relative overflow-hidden rounded-structural border border-border bg-content p-6 sm:p-8 md:p-12">
        ...
        <div className="relative max-w-2xl">
          <p className="text-[14px] font-semibold uppercase tracking-wide text-primary">{t('desfireStudio.title')}</p>
          <h2 className="mt-4 text-[34px] font-semibold leading-tight tracking-tight sm:text-[42px] md:text-[56px]">{t('desfireStudio.signInTitle')}</h2>
          <p className="mt-5 max-w-xl text-[16px] leading-7 text-muted-foreground">
            {t('desfireStudio.signInDescription')}
          </p>
      ...
      <div className="rounded-structural border border-border bg-content p-6 md:p-8">
        ...
        <h3 className="mt-6 text-[24px] font-semibold tracking-tight">{t('desfireStudio.operatorOnlyTitle')}</h3>
        <p className="mt-3 text-[14px] leading-6 text-muted-foreground">
          {t('desfireStudio.operatorOnlyDescription')}
        </p>
      </div>
    </section>
  );
